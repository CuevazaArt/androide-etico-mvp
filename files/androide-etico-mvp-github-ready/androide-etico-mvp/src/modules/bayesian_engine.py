...
class MotorBayesiano:
    ...
    Soporta variabilidad bayesiana: ruido controlado que hace
    que los scores varíen entre ejecuciones sin cambiar la
    acción elegida en la mayoría de casos.
    """
    ...
    def calcular_impacto_esperado(self, accion: AccionCandidata) -> float:
        ...
        Con variabilidad activa, perturba impacto y confianza
        para producir resultados ligeramente diferentes cada vez.
        """
        base = accion.impacto_estimado
        confianza = accion.confianza

        # Aplicar variabilidad si está activa
        if self.variabilidad:
            base = self.variabilidad.perturbar_impacto(base)
            confianza = self.variabilidad.perturbar_confianza(confianza)
